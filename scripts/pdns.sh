...
# echo -e "[Resolve]\nDNSStubListener=no" > /etc/systemd/resolved.conf # todo: append only mode or use sed
...
webserverkey=$(date +%s | sha256sum | base64 | head -c 32) # this is so insecure it's hilarious
...
# sed -i 's/launch=/launch=gsqlite3/' /etc/powerdns/pdns.conf # TODO: if statement before tee to check if script has already been run
